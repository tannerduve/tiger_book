...
let rec interpStm (s : stm) (t : table) : table =
  ...
  | AssignStm (i, e) -> 
    ...
    update t2 (i, n)
  | PrintStm l -> List.fold_left (fun acc e ->
  ...
  and interpExp (e : exp) (t : table) : int * table =
    ...
    | OpExp (e1, op, e2) -> 
        (match op with 
        | Plus -> 
          let t1 = interpExp e1 t in 
          let t2 = interpExp e2 (snd t1) in 
...
let rec insert (t : 'a tree) (k : key) (v : 'a) : 'a tree =
  match t with 
  | LEAF -> TREE (LEAF, (k, v), LEAF)
  | TREE (l, (x, v'), r) -> 
    if k < x then TREE(insert l k v, (x, v'), r)
    else if k > x then TREE(r, (x, v'), insert r k v)
    else TREE(l, (k, v), r)

let rec member (t : 'a tree) (k : key) : 'a =
  match t with 
  | LEAF -> failwith ("unbound id: " ^ k)
  | TREE(l, (x, v'), r) -> 
    if k < x then member l k 
    else if k > x then member r k 
    else v'
